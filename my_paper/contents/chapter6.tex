...
A/B 测试阶段完全复用第四章 4.1.2 中定义的九类网络环境及其 \texttt{pfctl + dummynet} 参数配置。各静态场景的关键参数（带宽上限、单向时延、丢包率、队列长度等）总览如图~\ref{fig:network-condition-parameter} 所示；其中 \texttt{fluctuating} 场景为动态时变场景，其带宽与时延参数沿时间轴的配置如图~\ref{fig:fluctuating-parameters} 所示，用于模拟带宽骤降与短时恢复等真实弱网的波动特征。
...
A/B 测试链路在 Node 服务端启动独立端口（默认 3001），两台终端设备打开 \texttt{ab\_test.html} 并在同一局域网内建立 WebRTC P2P 连接；每次实验固定使用同一段本地视频文件生成发送流，以减少内容复杂度差异对编码码率的影响。分组方式上，本研究采用“对照组 GCC / 实验组 RL + 限速 / 随机 50/50”三种模式中的随机模式作为默认配置，使每次连接在会话开始阶段随机分配到 GCC 或 RL 组，从而在多次重复试验中降低人为选择偏差。整体 A/B 测试流程（包含连接建立、场景切换、策略下发、统计采集与落盘等环节）如图~\ref{fig:ab-test-procedure} 所示。
...
图~\ref{fig:qoe-score}、\ref{fig:rtt-p95} 与 \ref{fig:loss-rate} 分别给出了各场景下 QoE 分数、RTT P95 与丢包率的分布对比，可直观观察两种策略在不同场景下的表现差异。
...
第一，\textbf{动作平滑性（cap\_delta）在所有场景中均显著下降}，下降幅度落在 42\%--73\% 区间，表明离线强化学习策略在动作序列上倾向于输出更稳定的带宽上限，能够有效抑制 GCC 在弱网反馈噪声下常见的“快速升降—再回退”的振荡行为。图~\ref{fig:cap-delta-cdf} 给出了全场景合并的 cap\_delta CDF，可见 RL 组的分布整体左移，即大幅度码率跳变出现的概率显著低于 GCC 组。基于 \texttt{output/qoe\_significance.csv} 的结果可发现，cap\_delta 在所有场景中均显著下降（p$\le$0.0012），说明该收益具有较强统计稳健性。
...
第三，\textbf{吞吐在多数场景中略有下降，但在 baseline 与 very\_bad 场景存在明显的带宽利用不足}。baseline 场景中 RTT、丢包与抖动基本不变，RL 仍降低了 18\% 的平均吞吐并使 QoE 下降 0.137，说明当前奖励权重与策略学习到的保守偏置可能导致“高质量网络下的过度限速”；very\_bad 场景中，RL 将 RTT P95 与丢包率分别降低 16\% 与 25\%，但吞吐下降 24\%，使综合 QoE 仅获得有限提升，表明在极端弱网下，吞吐收益项与惩罚项的权衡仍需进一步精细化。图~\ref{fig:throughput-delay-tradeoff} 以每个 client 片段为单位刻画了吞吐与时延的权衡关系，可以观察到 RL 组的样本点整体向“较低时延、略低吞吐”的方向偏移，与上述定量结论一致。
