...
在流媒体自适应传输层面，拥塞控制算法的有效性高度依赖于底层网络状态的精准观测。现代 WebRTC 架构普遍采用基于传输层范围的拥塞控制（Transport-wide Congestion Control, Transport-CC）扩展规范\cite{Sarker2021TransportCC}。与早期的接收端估算机制（如 REMB）不同，Transport-CC 将带宽预测的核心逻辑集中于发送端。
...
有别于传统 TCP 协议以最大化吞吐量为单一诉求\cite{Jacobson1988Congestion,Ha2008CUBIC,Cardwell2017BBR}，RTC 拥塞控制要求在带宽利用率与传输时延之间寻找严格的平衡点。其综合体验质量（Quality of Experience, QoE）的效用函数通常可形式化表征为如下非线性组合：
...
作为 WebRTC 默认的拥塞控制规范，Google 拥塞控制（Google Congestion Control, GCC）算法\cite{Carlucci2016GCC}采用基于延迟（Delay-based）与基于丢包（Loss-based）的双回路耦合控制机制。
